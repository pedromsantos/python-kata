...
objective as `smelly_mars_rover`, but with a more everyday domain: a shopping
cart (inspired by `shopping_cart`), built across the same
...
Unlike `smelly_tic_tac_toe` (which bakes in **production code smells** to
...
Unlike `smelly_mars_rover`, which is a single-file fixture, this kata is
...
None of the production files in this kata contain explanatory comments.
Finding and naming each blocker and each smell is part of the exercise --
this README intentionally does not say where they are.
...
| Domain (services)  | tested              | tests exist in `test/test_unit.py`, but nearly all are smelly           |
| Infrastructure     | tested              | tests exist in `test/test_integration.py`, but nearly all are smelly    |
...
than in `smelly_mars_rover`, spread across two layers.
...
2. **Fix the existing unit tests** -- everything in `test/test_unit.py`
...
3. **Fix the existing integration tests** -- same exercise, but for
   `test/test_integration.py`. Pay attention to how the same smell can look
