...
mission simulator (inspired by `mars_rover` and `esa_mars_rover`) built
...
Neither `kata.py` nor `test/test_kata.py` contain any explanatory comments.
...
| `CommandTranslator`, `MissionLog`, `Coordinate`               | tested   | tests exist in `test/test_kata.py`, but nearly all are smelly |
...
The 0%-covered part of `kata.py` has several deliberate testability blockers —
...
The tests in `test/test_kata.py` currently all pass, but the suite is
...
2. **Fix the existing tests** — everything in `test/test_kata.py` currently
   passes, but almost none of it protects real behavior. Detect each smell,
   then refactor it without ever weakening or deleting a test to make it pass.
