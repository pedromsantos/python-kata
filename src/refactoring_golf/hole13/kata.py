...
class Game:
    ...
    def play(self, player: Player, row: Row, column: Column):
...
class Board:
    ...
    def add_tile_at(self, tile: Tile):
        self._find_tile_by_coordinate(tile).update_player(tile.player)
    ...
    def _find_tile_by_coordinate(self, tile: Tile):
        found_tile = next((t for t in self.plays if t.has_same_coordinates_as(tile)), None)
    ...
    def _player_at(self, coordinate: Coordinate):
        return self._find_tile_by_coordinate(Tile(NO_PLAYER, coordinate)).player
    ...
    def _has_same_player(self, coordinate: Coordinate, other_coordinate: Coordinate):
        return self._find_tile_by_coordinate(Tile(NO_PLAYER, coordinate)).has_same_player_as(
            self._find_tile_by_coordinate(Tile(NO_PLAYER, other_coordinate))
        )
