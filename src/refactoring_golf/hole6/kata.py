...
class Board:
    ...
    def _is_row_full(self, row):
        return (
            self.tile_at(row, FIRST_COLUMN).symbol != EMPTY_PLAY
            and self.tile_at(row, SECOND_COLUMN).symbol != EMPTY_PLAY
            and self.tile_at(row, THIRD_COLUMN).symbol != EMPTY_PLAY
        )

    def _is_row_full_with_same_symbol(self, row):
        return (
            self.tile_at(row, FIRST_COLUMN).symbol == self.tile_at(row, SECOND_COLUMN).symbol
            and self.tile_at(row, THIRD_COLUMN).symbol == self.tile_at(row, SECOND_COLUMN).symbol
        )
